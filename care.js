(() => {
  ...
  const toyKinds = ['play-bubble','home-bed'];
  ...
  function injectLatestStyles() {
    ...
    style.textContent = `
      ...
      .jump-mini::before {
        ...
        background:linear-gradient(#9fe8bb,#58c98a);
        box-shadow:0 -7px 0 -5px #58c98a;
      }
      .jump-mini::after {
        content:"";
        position:absolute;
        ...
        width:10px;
        height:5px;
        border-top:2px solid #5b9f7a;
        border-radius:50%;
      }

      .slime.bubble-blow {
        animation:bubbleBlowBody 5.2s ease-in-out both;
      }
      .slime.bubble-blow .mouth {
        top:82px;
        width:18px;
        height:18px;
        border:3px solid #315247;
        border-radius:50%;
      ...
      .toy.play-bubble::before,
      .toy.play-bubble::after {
        content:"";
        position:absolute;
        border:3px solid #72b7ca;
        border-radius:50%;
        background:rgba(220,248,255,.2);
      }
      .toy.play-bubble::before {
        width:23px;
        height:23px;
        left:8px;
        bottom:2px;
        box-shadow:22px -20px 0 -5px rgba(220,248,255,.18),22px -20px 0 -2px #72b7ca;
      }
      .toy.play-bubble::after {
        width:15px;
        height:15px;
      ...
      .slime.play-jump-multi {
        animation:playJumpMulti 4.7s cubic-bezier(.22,.74,.28,1) both;
      }
      ...
      .slime.bed-sleeping {
        animation:bedSleepPose 6.8s ease-in-out both;
      }
      .slime.bed-sleeping .eye {
        height:4px;
        top:67px;
        border-radius:999px;
        box-shadow:none;
  ...
  function clearActivityClasses() {
    els.slime.classList.remove('bubble-blow','play-jump-multi','bed-sleeping');
  }
  ...
  function startBubblePlay(button) {
    ...
    setToy('play-bubble', 5200);
    window.setTimeout(() => particles('○', 4, 'bubble-pop'), 700);
    window.setTimeout(() => particles('○', 5, 'bubble-pop'), 1800);
    window.setTimeout(() => particles('○', 4, 'bubble-pop'), 3000);
    window.setTimeout(() => particles('○', 3, 'bubble-pop'), 4000);
  ...
  function startJumpPlay(button) {
    ...
    els.slime.classList.add('play-jump-multi');
    window.setTimeout(() => particles('·', 3, 'ball-pop'), 900);
    window.setTimeout(() => particles('·', 3, 'ball-pop'), 1900);
    window.setTimeout(() => particles('·', 3, 'ball-pop'), 2900);
    window.setTimeout(() => particles('·', 3, 'ball-pop'), 3900);
    commit(`${state.name} jumps around the playground again and again!`);

    activityTimer = window.setTimeout(() => {
      els.slime.classList.remove('play-jump-multi');
      els.slime.classList.add('idle');
    }, 4700);
  }
  ...
  document.querySelectorAll('[data-play]').forEach(button => {
    button.addEventListener('click', () => {
      if (button.dataset.play === 'bubble') startBubblePlay(button);
      else if (button.dataset.play === 'jump') startJumpPlay(button);
    });
  });
  ...
  function startBed(button) {
    ...
    activityTimer = window.setTimeout(() => {
      els.toy.classList.remove('show');
    }, 8400);
  }

  document.querySelectorAll('[data-home]').forEach(button => {
    button.addEventListener('click', () => {
      if (button.dataset.home === 'bed') startBed(button);
    });
  });
})();