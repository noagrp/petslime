(() => {
  ...
  function injectLatestStyles() {
    ...
    style.textContent = `
      ...
      @keyframes bedSleepPose{0%{transform:translate3d(var(--x),var(--y),0) scale(1)}12%{transform:translate3d(var(--x),calc(var(--y) + 8px),0) scale(1.1,.86)}20%,88%{transform:translate3d(var(--x),calc(var(--y) + 12px),0) scale(1.15,.76)}100%{transform:translate3d(var(--x),var(--y),0) scale(1)}}
  ...
  function startBubblePlay(button){
    ...
    els.slime.classList.add('bubble-blow');shadowAction('bubble',6200);
  ...
  function startJumpPlay(button){
    ...
    void els.slime.offsetWidth;els.slime.classList.add('play-jump-multi');shadowAction('jump',4700);
  ...
  function startTV(button){
    ...
    setToy('home-tv',5600);react('pet-react',1200,'pet-face');shadowAction('tv',5600);repeatParticles('·',2,'tv-note',4,900);commit(`${state.name} settles down beside the TV for a proper rest.`);
  }

  function startGame(button){
    clearActivityTimers();clearActivityClasses();selectOption(button);state.happiness+=8;state.energy-=3;state.fullness-=1;
    setToy('home-game',5800);stopThrow();clearReactionClasses();els.slime.classList.add('game-face');shadowAction('game',5800);
  ...
  function startShower(button){
    ...
    setToy('home-shower',4800);react('pet-react',1050,'pet-face');shadowAction('shower',4800);repeatParticles('|',5,'shower-drop',5,650);commit(`${state.name} stands under the shower and gets properly rinsed.`);
  }
  ...
  function startBed(button){
    ...
    setToy('home-bed',8400);els.sleepFx.classList.add('long-rest');replayFx(els.sleepFx,'show',6800);els.slime.classList.add('bed-sleeping');shadowAction('bed',8400);commit(`${state.name} settles on top of the bed... zzz.`);
