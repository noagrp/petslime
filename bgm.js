(() => {
  ...
  const PRIMARY='bgm/petslime-main-song.mp3';
  const FALLBACK='music/petslime-main-song.mp3';
  ...
  fetch(PRIMARY,{method:'HEAD',cache:'no-store'}).then(r=>{
    if(!r.ok)return;
    usingPrimary=true;
    if(audio.paused&&!activated){audio.src=PRIMARY;sourceReady=true}
  }).catch(()=>{}).finally(()=>{sourceReady=true});
  ...
  function setVolume(v){
    volume=Math.max(0,Math.min(40,Number(v)||0));
    audio.volume=volume/100;
    localStorage.setItem(VOL_KEY,String(volume));
  }
  function duck(ms=6500){
    clearTimeout(duckTimer);
    const normal=volume/100;
    audio.volume=Math.min(normal,.06);
    duckTimer=setTimeout(()=>{audio.volume=volume/100},ms);
  }
  ...
  function addControls(){
    ...
    title.textContent='BGM volume';
    value.textContent=`${volume}%`;
    label.append(title,value);
    const range=document.createElement('input');
    range.type='range';range.min='0';range.max='40';range.step='1';range.value=String(volume);
    range.setAttribute('aria-label','Background music volume');
    range.addEventListener('input',()=>{setVolume(range.value);value.textContent=`${volume}%`});
    wrap.append(label,range);

    menu.insertBefore(toggle,anchor||null);
    menu.insertBefore(wrap,anchor||null);
  }

  ['pointerdown','keydown','touchstart'].forEach(type=>document.addEventListener(type,activate,{once:true,passive:true}));
  document.addEventListener('visibilitychange',()=>{document.hidden?pause():play()});
  document.querySelector('[data-home="music"]')?.addEventListener('click',()=>duck(6500));

  window.petBgm={play,pause,setVolume,duck,get source(){return usingPrimary?PRIMARY:FALLBACK}};
  addControls();
})();