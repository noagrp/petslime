(() => {
  ...
  function sfx(name, variant = 0) {
    ...
    switch (name) {
      ...
      case 'whoosh': noise(.16, { filter: 'bandpass', frequency: 980 + variant * 110, q: 1.1, volume: .13 }); break;
      case 'roll-slow': tone(145, .42, { endFreq: 235, type: 'triangle', volume: .18 }); noise(.28, { filter: 'lowpass', frequency: 420, volume: .07 }); break;
      case 'roll-fast': tone(235, .2, { endFreq: 470, type: 'triangle', volume: .17 }); noise(.16, { filter: 'bandpass', frequency: 900, volume: .08 }); break;
      case 'dance-beat':
        ...
        break;
      case 'impact': tone(115, .08, { endFreq: 80, type: 'triangle', volume: .32 }); noise(.05, { frequency: 700, volume: .1 }); break;
      case 'sparkle': sequence([{freq:760,duration:.07},{freq:980,duration:.09}], 65); break;
      case 'hide': tone(360, .08, { endFreq: 260, volume: .2 }); laterSound(() => tone(520, .07, { endFreq: 650, volume: .18 }), 420); break;
      case 'dig':
        ...
        break;
      case 'bubble': tone(520, .13, { endFreq: 760, volume: .16 }); laterSound(() => tone(880, .06, { endFreq: 620, volume: .12 }), 520); break;
      ...
      case 'tv':
        ...
        break;
      case 'paper': noise(.13, { filter: 'highpass', frequency: 1500, volume: .11 }); laterSound(() => noise(.08, { filter: 'bandpass', frequency: 2200, q: .7, volume: .07 }), 95); break;
      case 'pour': noise(.46, { filter: 'bandpass', frequency: 1250, q: .55, volume: .1 }); laterSound(() => noise(.34, { filter: 'highpass', frequency: 2100, volume: .06 }), 110); break;
  ...
  function playMovement(kind) {
    clearScheduled();
    if (kind === 'jump') [[1080,0],[1880,1],[2730,2],[3710,1]].forEach(([t,v]) => laterSound(() => sfx('boing', v), t));
    else if (kind === 'sprint') [[520,0],[1120,1],[1780,2],[2440,1]].forEach(([t,v]) => laterSound(() => sfx('whoosh', v), t));
    else if (kind === 'skip') [[960,0],[2080,1],[3280,0]].forEach(([t,v]) => laterSound(() => sfx('skip-step', v), t));
    else if (kind === 'dodge') [[480,0],[1070,1],[1680,2],[2290,1],[2890,0]].forEach(([t,v]) => laterSound(() => sfx('whoosh', v), t));
    else if (kind === 'roll') { sfx('roll-slow'); laterSound(() => sfx('roll-slow'),950); laterSound(() => sfx('roll-slow'),1900); laterSound(() => sfx('roll-fast'),2650); laterSound(() => sfx('roll-fast'),3150); laterSound(() => sfx('roll-fast'),3550); }
    else if (kind === 'dance') { sfx('dance-beat',0); laterSound(() => sfx('dance-beat',1),720); laterSound(() => sfx('dance-beat',2),1450); laterSound(() => sfx('dance-beat',1),2200); laterSound(() => sfx('dance-beat',0),2870); }
  }

  function bindActionSounds() {
    document.querySelectorAll('[data-food]').forEach(button => button.addEventListener('click', () => { clearScheduled(); sfx('eat'); }));
    document.querySelectorAll('[data-play]').forEach(button => button.addEventListener('click', () => playMovement(button.dataset.play)));
    document.querySelectorAll('[data-interact]').forEach(button => button.addEventListener('click', () => {
      ...
      else if (kind === 'dig') { laterSound(() => sfx('dig',0),420); laterSound(() => sfx('dig',1),1000); laterSound(() => sfx('dig',2),2050); }
    ...
    document.querySelectorAll('[data-home]').forEach(button => button.addEventListener('click', () => {
      ...
      if (kind === 'tv') { sfx('tv'); laterSound(() => sfx('tv'),2050); }
      else if (kind === 'phone') { sfx('phone'); laterSound(() => sfx('phone'),850); laterSound(() => sfx('phone'),2050); laterSound(() => sfx('phone'),3350); }
      else if (kind === 'read') { sfx('paper'); laterSound(() => sfx('paper'),2850); }
      else if (kind === 'music') sfx('chime');
      else if (kind === 'shower') { sfx('pour'); laterSound(() => sfx('pour'),700); laterSound(() => sfx('pour'),1450); laterSound(() => sfx('pour'),2200); laterSound(() => sfx('pour'),2950); laterSound(() => sfx('pour'),3650); }
  ...
  if (slime) {
    slime.addEventListener('pointerdown', event => { clearScheduled(); dragStart = { x:event.clientX, y:event.clientY, time:performance.now() }; });
    slime.addEventListener('pointerup', event => {
      if (!dragStart) return;
      const dx=event.clientX-dragStart.x, dy=event.clientY-dragStart.y, distance=Math.hypot(dx,dy), elapsed=performance.now()-dragStart.time;
      dragStart=null;
      if (distance>38) { sfx('whoosh'); laterSound(() => sfx('land'),420); return; }
...
(() => {
  ...
  function snapshot(){ return {happiness:Number(state.happiness),fullness:Number(state.fullness),energy:Number(state.energy),coins:Number(state.coins)}; }
  function setFrom(before,delta){ state.happiness=before.happiness+(delta.happiness||0); state.fullness=before.fullness+(delta.fullness||0); state.energy=before.energy+(delta.energy||0); }
  function appendCoins(message,amount){ return amount>0 ? `${message} · +${amount} 🟡` : message; }
